...
def train_and_evaluate_lstm(df, features, target, time_steps, test_size, random_state):
    logging.info("--- STARTING LSTM PIPELINE ---")
    ...
    model = build_lstm_model(time_steps=time_steps, n_features=len(features))
    model.fit(X_train, y_train, epochs=15, batch_size=64, validation_split=0.1, verbose=1)
    
    predictions_scaled = model.predict(X_test)
    y_test_real = target_scaler.inverse_transform(y_test.reshape(-1, 1)).flatten()
    predictions_real = target_scaler.inverse_transform(predictions_scaled).flatten()
